...
#CC="%{_targetarch}_%{_realtargetcpu}-gcc" ./configure --prefix=/usr

%build
CC="%{__cc}" ./configure --host=ppc-linux --target=ppc
#make CC="%{__cc}"
make \
	WEBDIR=/home/httpd/html \
	BINDIR=%{_sbindir} prefix=/usr \
	CGIBINDIR=/home/httpd/cgi-bin \
	WEBGROUP=httpd

%install

mkdir -p $RPM_BUILD_ROOT/home/httpd/{cgi-bin,html,logs}
...
install -m555 thttpd $RPM_BUILD_ROOT%{_sbindir}
install -m444 thttpd.8 $RPM_BUILD_ROOT%{_mandir}/man8

install cgi-src/redirect $RPM_BUILD_ROOT/home/httpd/cgi-bin
install cgi-src/ssi $RPM_BUILD_ROOT/home/httpd/cgi-bin
install cgi-src/phf $RPM_BUILD_ROOT/home/httpd/cgi-bin
install cgi-src/redirect.8 $RPM_BUILD_ROOT%{_mandir}/man8
install cgi-src/ssi.8 $RPM_BUILD_ROOT%{_mandir}/man8

install -m555 extras/makeweb $RPM_BUILD_ROOT%{_sbindir}
install -m555 extras/htpasswd $RPM_BUILD_ROOT%{_sbindir}
install -m555 extras/syslogtocern $RPM_BUILD_ROOT%{_sbindir}
install -m444 extras/makeweb.1 $RPM_BUILD_ROOT%{_mandir}/man1
install -m444 extras/htpasswd.1 $RPM_BUILD_ROOT%{_mandir}/man1
install -m444 extras/syslogtocern.8 $RPM_BUILD_ROOT%{_mandir}/man8

#%pre
#
#grep '^httpd:' /etc/passwd >/dev/null || \
#	%{_sbindir}/adduser -r httpd
